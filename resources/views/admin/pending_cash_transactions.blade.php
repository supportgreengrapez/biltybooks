...
@section('content')
    ...
    @endif
    <form method="post" action="{{url('/')}}/admin-get-transaction-date/pending">
      {{csrf_field()}}
        <label for="">
        Start Date
      </label>
        ...
        <label for="">
          ...
          <button clas="btn btn-danger" type="submit">Search</button>
      </form>
        ...
        <tbody>
          ...
          @foreach($c as $ct)
            ...
            <td><a onclick="return confirm('Are you sure, you want to approve this cash payment??')" href="{{url('/')}}/admin-cash-transaction-approve/{{$ct->cid}}">approve</a> <a href="{{url('/')}}/admin-cash-transaction-decline/{{$ct->cid}}">decline</a></td>
            </tr>
